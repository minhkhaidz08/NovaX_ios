...
private struct FeatureRow: View {
    ...
    private func iconFor(_ title: String) -> String {
        if title.localizedCaseInsensitiveContains("Aimlock") { return "target" }
        if title.localizedCaseInsensitiveContains("Safe") { return "checkmark.shield" }
        if title.localizedCaseInsensitiveContains("Body") { return "figure.walk" }
        return "puzzlepiece"
    }
}
...
private struct UsageGuideCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            NovaCard(backgroundColor: nova.card.opacity(0.5), cornerRadius: 16) {
