...
// MARK: - Logo image (loaded explicitly from bundle — avoids asset-catalog lookup)

extension Image {
    static func novaLogo() -> Image {
        guard let path = Bundle.main.path(forResource: "nova_logo", ofType: "png"),
    ...
    static func ffIcon() -> Image {
        guard let path = Bundle.main.path(forResource: "ff", ofType: "jpg"),
              let ui = UIImage(contentsOfFile: path) else {
...
struct NovaLoading: View {
            ...
            ZStack {
                ForEach(0..<12, id: \.self) { i in
                    ...
                    Circle()
                        ...
                        .frame(width: _size(size), height: _size(size))
                        .offset(x: _step(r * cos(rad)), y: _step(r * sin(rad)))
                }
                Circle()
                    .stroke(
    ...
    private func _size(_ value: Double) -> CGFloat { CGFloat(value) }
    private func _step(_ value: Double) -> CGFloat { CGFloat(value) }
}
