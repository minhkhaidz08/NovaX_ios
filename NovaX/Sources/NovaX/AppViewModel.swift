...
final class AppViewModel: NSObject, ObservableObject {
    ...
    func login() async {
        ...
        let secret = Obf.s([235,202,211,196,253,136,196,193,215], 0xA5)
    ...
    private func performVerify(url: URL) async -> VerifyResult {
        ...
        do {
            ...
            guard !data.isEmpty else {
                return .invalid(status: "", message: "Server returned empty response (HTTP \(httpResp.statusCode))")
            }

            let json: [String: Any]
            do {
                guard let obj = try JSONSerialization.jsonObject(with: data) as? [String: Any] else {
                    return .invalid(status: "", message: "Server response error (invalid JSON)")
                }
                json = obj
            } catch {
                return .invalid(status: "", message: "Server response error (invalid JSON)")
